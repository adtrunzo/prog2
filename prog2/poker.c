...
int main(int argc, char** argv) {
    ...
    int trialsPerProcess = cnt / size;

    for (int i = 0; i < trialsPerProcess; i++) {
        srand((unsigned int)(MPI_Wtime() * 1000000) + rank);
    ...
    int* allStraightFlushes = NULL;
    if (rank == 0) {
        end_time = MPI_Wtime();
        allStraightFlushes = (int*)malloc(size * sizeof(int));
    }

    MPI_Gather(&straightFlushes, 1, MPI_INT, allStraightFlushes, 1, MPI_INT, 0, MPI_COMM_WORLD);

    if (rank == 0) {
        int totalStraightFlushes = 0;
        for (int i = 0; i < size; i++) {
            totalStraightFlushes += allStraightFlushes[i];
        }

        percent = (float)totalStraightFlushes / (float)cnt * 100.0;

        printf("We found %d straight flushes out of %d hands or %f percent.\n", totalStraightFlushes, cnt, percent);
        printf("Elapsed time: %f seconds\n", end_time - start_time);

        free(allStraightFlushes);
    }
